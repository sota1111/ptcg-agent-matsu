"""Matsu KPI history display + latest-vs-previous trend (SOT-1708).
...
FLAT_EPS = {
    "mirror_winrate_vs_greedy": 0.005,
    "self_deck_out_loss_rate": 0.005,
    "fault_total": 0,
...
def compare(prev: dict, latest: dict) -> dict:
    ...
    for name in KPI_DIRECTIONS:
        ...
        out[name] = {
            "prev": p, "latest": l,
            "delta": round(l - p, 4) if p is not None and l is not None
            else None,
...
def print_history(history: list) -> None:
    cols = ("ts", "git_sha", "issue", "source", "N",
            "winrate", "ci95", "deckout", "faults", "ms/move")
    rows = []
    for r in history:
        wr = (r.get("kpis") or {}).get("mirror_winrate_vs_greedy", {})
        ci = wr.get("ci95")
        rows.append((
            ...
            fmt(kpi_value(r, "self_deck_out_loss_rate")),
